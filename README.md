...
- **M17-Demodulation in `pluto_advanced_rx` mit echter Hardware verifizieren** — RX ist implementiert (neuer "M17"-Modus, nutzt `m17.m17_decoder`/`m17.codec2_decoder` aus dem bereits vendorten gr-m17 plus `digital.symbol_sync_ff`, da `m17.symbol_sync` im gr-m17-Quellcode zwar vorhanden aber in diesem Checkout nicht gebaut ist) und per Software-Loopback bit-exakt bestätigt (echte TX→RX-Kette inkl. FM-Modulation/-Demodulation, Rufzeichen und Audio korrekt dekodiert). Noch offen: echter Hardware-Test (Pluto TX → RTL-SDR RX bei 432,15 MHz, niedrige Sendeleistung) sowie ein Cross-Check gegen eine unabhängige M17-Implementierung.
