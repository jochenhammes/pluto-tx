...
**Verifiziert**: Offline-Rundlauftest (verständliche Sprache, Echtzeitfaktor ≈0,03–0,05), realer Sendetest mit dem Pluto — erfolgreich empfangen und dekodiert sowohl mit SDR++ + externem audio-basiertem RADE-Decoder als auch mit `pluto_advanced_rx` selbst (Sprache gut verständlich). Feintuning von Frequenz/Gain war dafür nötig (siehe ToDo).
...
- **RADE-Empfang in `pluto_advanced_rx` bestätigt, braucht aber viel manuelles Feintuning** — Nutzer hat real mit dem Pluto gesendet und sowohl mit SDR++ + externem audio-basiertem Decoder als auch mit `pluto_advanced_rx` selbst erfolgreich empfangen/dekodiert (Sprache gut verständlich). Bis zum erfolgreichen Decode war aber deutliches manuelles Nachjustieren von Frequenz und Gain nötig. **ToDo: Unterstützung beim Feintuning einbauen** — z.B. Frequenz automatisch anhand von `rade_decoder.freq_offset_hz` nachführen (fine-tune-Offset direkt aus dem laufenden Sync-Zustand ableiten statt rein manuell im Wasserfall zu suchen), und/oder eine geführte Gain-Anpassung (z.B. SNR/Sync-Status als Live-Feedback beim Drehen am Gain-Regler, oder ein automatischer Sweep). Bisher zeigt die GUI Sync/Freq-Offset/SNR nur passiv an (siehe RADE-Abschnitt oben) — die Werte sind vorhanden, werden aber nicht genutzt, um die Bedienung selbst zu erleichtern.
