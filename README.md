...
Beide Apps haben in der GUI ein Textfeld ("Device (hostname or IP)") plus einen Connect/Disconnect-Button. Standardmäßig wird beim Start automatisch mit `config.DEFAULT_URI` (`ip:plutoplus.local`) verbunden. Für einen zweiten Pluto im selben LAN, oder um explizit auf USB umzuschalten, im Feld einfach eintragen:

- Bloßer Hostname oder IP (z.B. `192.168.1.50`) — wird automatisch zu `ip:192.168.1.50`.
- Eine volle libiio-URI (`usb:1.5.5`, `ip:anderer-pluto.local`, ...) — wird unverändert übernommen.
...
- **Kein automatischer Geräte-Scan** — die Gerätewahl (siehe oben) ist rein manuell (Hostname/IP eintippen), es gibt kein Dropdown mit erkannten Contexts. Für "welche Plutos sind gerade im LAN?" bräuchte es einen `iio_scan_context`-Aufruf im Hintergrund. Der native USB-Backend könnte außerdem den RX-Durchsatz über die aktuell gemessenen ~4,7-4,9 MSa/s (IIOD-Netzwerkprotokoll) hinaus verbessern und 5/10-MHz-RX-Bandbreiten wieder nutzbar machen.
